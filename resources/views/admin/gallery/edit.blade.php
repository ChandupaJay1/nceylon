...
@section('title', 'Admin - Edit Gallery Image')
...
<div class="min-h-screen bg-surface flex">
    ...
    <div class="flex-1 lg:ml-64 pt-10 pb-16 px-6 lg:px-10">
        ...
        <div class="mb-10">
            ...
            <h1 class="text-3xl font-heading font-black text-secondary">Edit Gallery Image</h1>
        </div>

        <!-- Form -->
        <form action="{{ route('admin.gallery.update', $gallery) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-lg p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" id="title" name="title" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-primary @error('title') border-red-500 @enderror" placeholder="e.g., Cinnamon Processing" value="{{ old('title', $gallery->title) }}">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-primary @error('description') border-red-500 @enderror" placeholder="Image description...">{{ old('description', $gallery->description) }}</textarea>
                @error('description') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            <!-- Current Image -->
            <div>
                <label class="block text-sm font-black text-secondary mb-2">Current Image</label>
                <img src="{{ asset($gallery->image_path) }}" alt="{{ $gallery->title }}" class="w-full max-w-md rounded-lg object-cover border border-gray-300">
            </div>

            <!-- Image Upload -->
            <div>
                <label for="image" class="block text-sm font-black text-secondary mb-2">Replace Image (Optional)</label>
                <div class="relative">
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" id="sort_order" name="sort_order" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-primary @error('sort_order') border-red-500 @enderror" placeholder="0" value="{{ old('sort_order', $gallery->sort_order) }}">
            ...
            <div class="pt-4 border-t border-gray-200">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" class="w-5 h-5 rounded border-gray-300 text-primary focus:ring-primary" @checked(old('is_active', $gallery->is_active))>
