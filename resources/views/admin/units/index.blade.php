...
<div class="min-h-screen bg-surface flex">
    ...
    <div class="flex-1 lg:ml-64 pt-10 pb-16 px-6 lg:px-10">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-10">
            <div>
                <h1 class="text-3xl font-heading font-black text-secondary">Units</h1>
                <p class="text-secondary/50 text-sm mt-1">Manage product measurement units</p>
            </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-surface transition-colors">
                        <td class="px-6 py-4 text-sm text-secondary/40 font-semibold">{{ $unit->id }}</td>
                        <td class="px-6 py-4 font-black text-secondary">{{ $unit->name }}</td>
                        <td class="px-6 py-4 text-sm text-secondary/60">{{ $unit->sort_order }}</td>
                        <td class="px-6 py-4">
                            @if($unit->is_active)
                        ...
                        <td class="px-6 py-4 text-sm text-secondary/60 font-semibold">
                            {{ \App\Models\Product::where('unit', $unit->name)->count() }}
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-3">
                                ...
                                <form action="{{ route('admin.units.destroy', $unit) }}" method="POST"
                                      onsubmit="return confirm('Delete \'{{ $unit->name }}\'?')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-16 text-center text-secondary/40">
                            <i class="fas fa-weight-hanging text-4xl mb-3 block opacity-20"></i>
                            No units yet. <a href="{{ route('admin.units.create') }}" class="text-primary font-semibold">Add one now</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
