...
<div class="min-h-screen bg-surface flex">
    ...
    <div class="flex-1 lg:ml-64 pt-10 pb-16 px-6 lg:px-10">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-10">
            <div>
                <h1 class="text-3xl font-heading font-black text-secondary">Categories</h1>
                <p class="text-secondary/50 text-sm mt-1">Manage product categories</p>
            </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-surface transition-colors">
                        <td class="px-6 py-4 text-sm text-secondary/40 font-semibold">{{ $category->id }}</td>
                        <td class="px-6 py-4 font-black text-secondary">{{ $category->name }}</td>
                        <td class="px-6 py-4 text-sm text-secondary/60">{{ $category->sort_order }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-3">
                                ...
                                <form action="{{ route('admin.categories.destroy', $category) }}" method="POST"
                                      onsubmit="return confirm('Delete \'{{ $category->name }}\'? Products using this category will keep their current value.')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-16 text-center text-secondary/40">
                            ...
                            No categories yet. <a href="{{ route('admin.categories.create') }}" class="text-primary font-semibold">Add one now</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
